...
*/
int Input_Output_Manager::read_in_a_line( vector<itemtype>& basket )
{
   ...
   do
   {
      ...
      if( pos ) basket.push_back( (itemtype) item );
   }
...
unsigned long Input_Output_Manager::find_frequent_items( 
   const unsigned long min_supp, vector<unsigned long>& support_of_items )
{
   ...
   vector<itemtype>::iterator it_basket;
   ...
   for( itemtype edgeIndex = 0; edgeIndex < temp_counter_vector.size(); 
	edgeIndex++ )
      if( temp_counter_vector[edgeIndex] >= min_supp )
	 temp_set.insert(
...
void Input_Output_Manager::basket_recode( 
   const vector<itemtype>& original_basket, vector<itemtype>& new_basket )
{
   new_basket.clear();
   for( vector<itemtype>::const_iterator it_basket = original_basket.begin(); 
